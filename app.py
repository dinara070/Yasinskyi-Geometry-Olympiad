...
# === METHODOLOGICAL (ВИПРАВЛЕНО) ===
elif current_page == "method":
    st.title(t["menu_items"]["method"])
    st.info("Розділ для викладачів та студентів кафедри.")
    
    # --- 1. База даних тем та задач ---
    topic_database = {
    ...
    with st.form("method_gen"):
        ...
        # Кнопка сабміту В СЕРЕДИНІ форми
        submitted = st.form_submit_button("Згенерувати картку")

    # ЛОГІКА ТА ЗАВАНТАЖЕННЯ — ЗА МЕЖАМИ ФОРМИ (ВИПРАВЛЕНО)
    if submitted:
        ...
        st.success("Картку успішно згенеровано!")
        
        # Тепер кнопка завантаження працюватиме, бо вона не в формі
        st.download_button(
